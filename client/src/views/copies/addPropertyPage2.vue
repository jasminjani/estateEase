          ...
          <v-card-text>
            <v-form>
              ...
              <div id="outerDiv">
                ...
                >
                  <v-text-field
                    v-model="jobsCount[index].name.value"
                    :error-messages="jobsCount[index].name.errorMessage"
                  ...
                  ></v-text-field>
                  <v-textarea
                    v-model="jobsCount[index].description.value"
                    :error-messages="jobsCount[index].description.errorMessage"
                  ...
                  <v-file-input
                    v-model="jobsCount[index].photos.value"
                    :error-messages="jobsCount[index].photos.errorMessage"
                    :name="`jobimage_${index}`"
                    label="Upload image"
                    accept="image/*"
                    chips
                    multiple
...
const jobSChemas = {
  //   jobsCount.forEach((element, i) => {
  //     jobname_0: yup.string().required("job name is required").trim(),
  //   jobimage_0: yup.mixed().required("Image is requireeed"),
  //   job_description_0: yup.string().required(),
  // }),
  jobname_0: yup.string().required("job name is required").trim(),
  jobimage_0: yup.mixed().required("Image is requireeed"),
  job_description_0: yup.string().required().trim(),
};
...
const { handleSubmit } = useForm({
  validationSchema: yup.object({
    ...
    ...jobSChemas,
    // jobname_1: yup.string().required("job name is required").trim(),
    // jobimage_1: yup.mixed().required("Image is requireeed"),
    // job_description_1: yup.string().required(),
  }),
});

const jobsCount = reactive([{ name: "", description: "", photos: [] }]);
...
jobsCount.forEach((element, i) => {
  jobsCount[i].name = useField(`jobname_${i}`);
  jobsCount[i].description = useField(`job_description_${i}`);
  jobsCount[i].photos = useField(`jobimage_${i}`);
});
...
const addMoreJobs = async () => {
  ...
  jobsCount.push({ name: "", description: "", photos: [] });
  // jobsCount[1].name = useField("jobname_1");
  // jobsCount[1].description = useField("job_description_1");
  // jobsCount[1].photos = useField("jobimage_1");
};
